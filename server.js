...
app.post('/api/usuarios', async (req, res) => {
    try {
        ...
        if (!nuevoUsuario.deporte) {return res.status(400).json({ error: 'El campo deporte es requerido' });}
...
app.post('/api/tablas', async (req, res) => {
    try {
        ...
        if (tablas) {
            ...
            await tablas.save();
        } else {tablas = await Tablas.create(nuevasTablas);}
...
app.delete('/api/tablas/:tipo/:index', async (req, res) => {
    try {
        ...
        if (tipo === 'general') {tablas.generales.splice(parseInt(index), 1);
        } else {
