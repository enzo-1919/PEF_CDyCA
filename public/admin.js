...
function mostrarCampoUsuario() {
    ...
    if (tipo === 'individual') {
        campo.style.display = 'block';} else {
    ...
    Object.keys(tablas.individuales).forEach(usuario => {
        tablas.individuales[usuario].forEach((tabla, index) => {
            if (tabla.deporte === deporteActual) {
                ...
                contenedor.appendChild(card);}});});
    if (contenedor.children.length === 0) {contenedor.innerHTML = '<p>No hay tablas creadas aún para este deporte</p>';}}
...
async function eliminarTabla(tipo, index) {
    ...
    const tablas = await cargarTablas();
    if (tipo === 'general') {
        tablas.generales.splice(index, 1);} else {
...
function crearTablaEjercicios(lista, indexLista) {
    ...
    let maxSemanas = 1;
    lista.ejercicios.forEach(ej => {if (ej.semanas && ej.semanas.length > maxSemanas) {maxSemanas = ej.semanas.length;}});
    ...
    lista.ejercicios.forEach((ejercicio, indexEjercicio) => {
        ...
        if (!ejercicio.semanas || !Array.isArray(ejercicio.semanas)) {ejercicio.semanas = [{ series: '', repeticiones: '', peso: '', t1: '', t2: '', t3: '' }];}
        while (ejercicio.semanas.length < maxSemanas) {ejercicio.semanas.push({ series: '', repeticiones: '', peso: '', t1: '', t2: '', t3: '' });}
...
function agregarSemanaTodos(indexLista) {
    ...
    lista.ejercicios.forEach(ejercicio => {
        if (!ejercicio.semanas) {ejercicio.semanas = [];}
...
function agregarEjercicio(indexLista) {
    ...
    const nuevoEjercicio = {
        nombre: nombre,
        semanas: [{ series: '3', repeticiones: '10', peso: '40', t1: '', t2: '', t3: '' }]};
